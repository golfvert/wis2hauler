...
This document covers every key in the YAML configuration file, the five roles and how they combine on a replica, the always-on heartbeat/leader-election mechanism underneath the singleton roles, and the runtime HTTP admin API.
...
```bash
bun src/main.ts <configuration.yml> [-d role[,role...]]...
```
...
`-d role[,role...]` (repeatable, comma-separable) sets the **static, startup-only** debug-category baseline — `SUBSCRIBER`, `DOWNLOADER`, `CLEANER`, `REPORTER`, `REPLAYER`, or `ALL`. It's the only config that comes from the CLI rather than the YAML file. Everything else that can change without a restart goes through the runtime HTTP admin API described below (`GET /get`, `POST /set`) — there is no live-reloaded debug file or any other second live-mutation channel.

A subset of YAML settings can also be changed at runtime through `POST /set` (and, for replay, `POST /replayer`) without restarting the process — these are marked **runtime-settable** below. A runtime change is **not** written back to the YAML file and is **per-process** (not shared with other replicas) — restarting reloads the file's original values. See "Runtime admin API" at the end of this document for the mechanics.
...
Comma-separated list of roles this replica performs: any combination of `SUBSCRIBER`, `DOWNLOADER`, `CLEANER`, `REPORTER`, `REPLAYER`. Each role listed requires its matching section to be present (`SUBSCRIBER` → `subscriber:`, `DOWNLOADER` → `downloader:`, `REPLAYER` → `replayer:`) — a missing section for an active role is a hard error at startup. `CLEANER` without a `cleaner:` section is only a warning (files simply never get cleaned).

### `global.worker` — required

A unique name for this replica within the shared Redis store. Used throughout Redis key names (e.g. `wis2gc:downloader:worker3:...`) and in the leader-election heartbeat hash. **Must be distinct across every live replica** — two replicas sharing a worker name will corrupt each other's queue tracking and election state.
...
The name of the Redis stream this replica's SUBSCRIBER/DOWNLOADER work-queue reads from and writes to. All replicas sharing a `queue` value participate in the same Redis Streams consumer group and process the same backlog; replicas with different `queue` values are completely independent of each other.
...
Set `true` when this deployment is operating as a WIS2 Global Cache rather than a national centre/observer node. When `true`, the subscriber applies an extra rule: a message whose `properties.cache` is `false` is still forwarded to the local broker (so other local consumers still see it) but is **not** queued for download — this is what respects a data provider's explicit "don't cache this globally" flag.

### `global.centre-id` — optional

This replica's own WIS2 centre identifier, used as part of the MQTT client id built for every broker connection this process opens (`<centre-id>-<worker>-<label>`, e.g. `<centre-id>-<worker>-GB1`).

### `global.local-broker` — optional (warning if missing/empty)

Up to two local MQTT brokers, wired to `PUB1`/`PUB2`, that `SUBSCRIBER`'s publish-only outcomes and `DOWNLOADER`'s completion notifications get republished to. See [`deployment.md`](deployment.md#local-mqtt-brokers-and-publish-only-outcomes). Same broker object shape as `subscriber.global-broker` below (`broker`, `username`, `password`, `version`, `verifycert`).

### `global.http-port` — optional, default `8080`

The port this replica's one shared HTTP admin server binds to. Always bound, on every replica, regardless of active roles — the admin API (`/get`, `/set`) is never role-gated, matching the original Node-RED admin server. There is no auto-discovery or self-registration of this port with anything (no dynamic Traefik registration) — set it explicitly for a real deployment and wire it into your own reverse proxy or firewall rule, same as you would for aria2's own RPC port.
...
Up to two WIS2 Global Brokers, wired to `GB1`/`GB2`. Messages from GB1 are processed immediately; GB2 messages are deliberately delayed 2 seconds before dedup, so that when the same notification arrives on both, the GB1 copy wins and the GB2 copy is dropped as a duplicate. If GB1 is unreachable, GB2 still works on its own. Same broker object shape as `global.local-broker`.
...
Rules that force a message to publish-only (no download), by topic match and/or by exceeding a `max-length` in bytes — used for content you want republished locally but never actually cached.
...
Normally left unset. When non-null, an extra `replay/a/wis2/<value>/#` subscription is added — set automatically by a `REPLAYER`-triggered replay, not usually hand-edited in the file.

---

## `downloader`

Required iff `DOWNLOADER` is in `global.roles`.

### `downloader.aria-secret` / `downloader.aria-url` — required

The `--rpc-secret` aria2 was started with, and its JSON-RPC WebSocket endpoint (`ws://` or `wss://`). One real WebSocket connection is opened and reused for every download this replica manages.

### `downloader.aria-inqueue` — required

The maximum number of downloads queued or in-flight at once — the main backpressure knob. Once reached, this replica stops pulling new work off the Redis stream until a slot frees up. Typical values for a busy global cache: 100–500; too high overwhelms aria2, too low leaves bandwidth idle.
...
This cache's own WIS2 identifier, stamped into `properties.global-cache` on every notification this replica publishes after a successful download. Consumed by other caches' `subscriber.priority-global-cache` lists.
...
### `downloader.s3access` — required iff `rename-to: "s3"`
...
Per-topic HTTP Basic credentials for authenticated data sources, keyed by a WIS2 topic that must start with `origin/` and carry `recommended` at level 6. Seeded into the shared Redis hash at startup (added-to, not replacing, whatever's already there from other replicas or prior runtime changes) and re-synced from Redis every 10 seconds.
...
The WIS2-GREP OGC API Processes endpoint this replica POSTs to when a replay is triggered via `POST /replayer`.
...
Every replica carrying `SUBSCRIBER` or `DOWNLOADER` runs concurrently and independently — there's no election or "one active at a time" gating for these two. Coordination happens through Redis Streams consumer groups instead: multiple `DOWNLOADER` replicas sharing the same `global.queue` value pull disjoint entries off the same stream and each does its own share of the work. Run as many of these as your throughput needs, limited only by Redis and your downstream bandwidth.

`SUBSCRIBER` and `DOWNLOADER`, when both active on the same replica, share the same `PUB1`/`PUB2` local-broker connections rather than each opening their own (the process's orchestrator, `src/main.ts`, owns opening these once per process). `SUBSCRIBER`'s own `GB1`/`GB2` upstream connections are opened inside a dedicated try/catch, so an unreachable global broker only fails the `SUBSCRIBER` role on that replica, not the whole process.

### Singleton roles: CLEANER, REPORTER, REPLAYER

These three are "active-standby": every replica carrying one of them runs the same election poll and only the currently-elected primary actually does the role's work (schedules file deletions, serves `/reporter/primary`'s 200, answers `POST /replayer`). A non-primary replica's copy of the role sits idle, ready to take over the moment the current primary's heartbeat goes stale. Run more than one replica carrying the same singleton role purely for failover — running many of them doesn't increase throughput the way it does for `SUBSCRIBER`/`DOWNLOADER`.

### The always-on heartbeat and leader election

**Every** replica, regardless of which roles it carries, writes its own heartbeat fields into one shared Redis hash (`wis2gc:configuration`) every **2 seconds** (after an initial 3-second delay): its own worker name, a process UUID, which of the five roles it carries, whether it's running in S3-relocate mode, and — if it carries `SUBSCRIBER` — the topics it's currently subscribed to. This heartbeat has no role gating at all; it's unconditional infrastructure every process participates in.

Each of `CLEANER`, `REPORTER`, and `REPLAYER` separately polls that same shared hash every **10 seconds** and decides its own primary/secondary status: among every replica whose heartbeat is fresher than **8 seconds** old and which flags that role as `true`, the one with the lexicographically lowest process UUID is primary; everyone else (for that role) is secondary. Every poll also reaps (HDELs) the fields of any worker whose heartbeat has gone stale for **60 seconds or more** — any replica finding stale fields deletes them, so this is safe to run concurrently across every replica without coordination.

`CLEANER` additionally derives a cluster-wide `cleaning-needed` flag from the same heartbeat data: cleaning proceeds unless *every* currently-alive `DOWNLOADER`-carrying replica is running in S3-relocate mode (a mixed or all-local-disk fleet always cleans; a fleet with no downloader replicas at all also defaults to cleaning, fail-safe).
...
Every replica exposes this on `global.http-port` (default `8080`), regardless of active roles. Changes take effect immediately in the running process and are **not** written back to the YAML file — a restart reverts to the file's values. State is **per-process**, not shared across replicas (the same as the original Node-RED `global` context was never cross-instance either).

### `GET /get[?key=<name>]`

Without `key`, returns every field the replica's active roles are allowed to see. With `key`, returns just that field (`400` for an unknown key, `403` if the current roles aren't allowed to see it).
...
| `credentials` | `DOWNLOADER` | `{"op": "create"\|"update"\|"delete", "topic": "...", "username"?: "...", "password"?: "..."}` — applied as a real Redis HSET/HDEL, picked up by every `DOWNLOADER` replica within its next 10s sync. |
| `debug` | any | Replaces the dynamic debug-category set wholesale (e.g. `["SUBSCRIBER", "DOWNLOADER"]`). |

Example — pause a replica for maintenance:
...
Only the currently-elected `REPLAYER` primary processes this; every other replica returns `403` immediately. Use `GET /replayer/primary` first to find which one that is.
...
`200` if this replica currently holds that role's election, `404` otherwise — use these to find which replica is actively serving metrics or replay requests.
