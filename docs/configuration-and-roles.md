...
| `consumer.ts`, raw-stream health check | `Redis` | `hauler-redis-*.warn.log` (and `.info.log` for the matching cleared events) | Not per-message: a periodic (every `healthCheckIntervalMs`, default 5s) check of the raw Redis stream's own `XLEN` and oldest surviving entry ID, added 2026-09-21 after a production incident where `MAXLEN ~` silently trimmed unread entries out from under the consumer with zero error. Warns once when the stream's length reaches 80% of `RAW_STREAM_MAXLEN` (early warning, may resolve on its own) and, separately, once when the stream's oldest surviving entry has actually overtaken the consumer's own read cursor (confirmed loss — entries were trimmed before being read). Each warning logs once on transition and once again when it clears, not on every check. |
