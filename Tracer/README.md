...
Trace one `data_id` or `wnm.id` across every `hauler-*.log` file a Hauler deployment writes — `Received`/`Filter`/`Decision`/`Duplicate` on SUBSCRIBER, `Aria`/`Ack`/`Correct ?`/`Re-queue`/`Update`/`Output - *`/`Duplicates`/`Poll Error` on DOWNLOADER, across every worker — printed as one chronological narrative instead of a per-file `grep`.
